...
from roc_threshold import compute_roc_curve_from_dir, compute_threshold


def main() -> None:
    # Example files.  Update these paths to point to your own images.
    original_image = 'images\\0032.bmp'
    ...
    # attacked = attacks(watermarked_path, 'awgn', [20])
    attacked = attacks(watermarked_path, 'jpeg', [20])
    ...
    # Step 4: Compute and plot a ROC curve using the ROC procedure on a
    # single image.  In practice you should provide more images and
    # increase n_samples for a smoother curve.  The ROC plot will be
    # saved to 'roc_curve_demo.png'.
    
    # OLD FUNCTION CALL FROM roc_threshold_old.py
    # from roc_threshold import compute_roc_curve_random
    # tau = compute_roc_curve_random(watermark_file, [original_image], attack_name='awgn', attack_params=10,
    #                                 fpr_limit=0.1, n_samples=5, plot_file='roc_curve_demo.png')
    # print(f"Estimated similarity threshold (τ) for FPR ≤ 0.1: {tau:.4f}")

    from roc_threshold import compute_roc_curve_from_dir
    tau = compute_roc_curve_from_dir('src\\acme.npy', './images', n_samples=5,
                                 fpr_limit=0.1, plot_file='roc_all.png')
    print("Estimated τ:", tau)

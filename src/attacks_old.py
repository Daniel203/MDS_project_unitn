...
def randomized_attack(img: np.ndarray) -> np.ndarray:
    """Apply a random attack with random parameters to the input image."""
    attack_types = ['awgn', 'blur', 'sharp', 'jpeg', 'resize', 'median']
    attack = np.random.choice(attack_types)

    # call the attacks function with random parameters
    if attack == 'awgn':
        param = np.random.uniform(5, 20)  # standard deviation between 5 and 20
    elif attack == 'blur':
        param = np.random.choice([3, 5, 7])  # odd kernel size
    elif attack == 'sharp':
        param = None  # no parameter needed
    elif attack == 'jpeg':
        param = np.random.randint(30, 90)  # quality between 30 and 90
    elif attack == 'resize':
        param = np.random.uniform(0.5, 1.5)  # scale between 0.5 and 1.5
    elif attack == 'median':
        param = np.random.choice([3, 5, 7])  # odd kernel size
    else:
        raise ValueError("Unknown attack type")
    
    return attacks(img, attack, [param])

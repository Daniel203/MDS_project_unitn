...
def randomized_attack(img_input: Union[str, np.ndarray]) -> np.ndarray:
    """Apply a single random attack to the given image.

    This helper selects one of the permitted attacks at random and
    generates an appropriate random parameter.  The input may be a
    filename (string) or a NumPy array representing a grayscale image.
    When a filename is provided, the image will be read from disk.

    Args:
        img_input: Either a path to a grayscale image on disk or a
            NumPy array containing the image data.  Images must be
            single-channel (grayscale) with values in the 0–255 range.

    Returns:
        A new NumPy array containing the attacked image.  The input
        image is not modified.
    """
    # Load the image if a filename is provided.  This allows callers
    # to pass either a path or an already-loaded array.  If the
    # provided array has multiple channels we convert it to grayscale.
    if isinstance(img_input, (str, bytes)):
        img = cv2.imread(img_input, cv2.IMREAD_GRAYSCALE)
        if img is None:
            raise FileNotFoundError(f"Could not read input image: {img_input}")
    else:
        img = img_input.copy()
        if img.ndim == 3:
            # Convert colour images to grayscale for consistency
